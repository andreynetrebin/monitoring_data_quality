import pandas as pd
from export_data.csv_export import export_data_to_csv, load_acc_ids_from_csv, merge_csv_files
...
def perform_check_and_export(db_ops, db_ops_integrating, cur, check_query, check_name, report_lines):
    ...
    if count > 0:
        export_data_to_csv(cur, check_query, f'{check_name}.csv')
        acc_ids = load_acc_ids_from_csv(f'{check_name}.csv', encoding='cp1251')
        ...
        if acc_ids:
            ...
            """
            integra_csv_file = f'{check_name}_integra.csv'
            ...
            result_csv_file = f'{check_name}_доб_интеграция.csv'
            merge_csv_files(f'{check_name}.csv', integra_csv_file, result_csv_file, encoding='cp1251')
